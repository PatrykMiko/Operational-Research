...
class Objective:
    ...
    def substitute(
        ...
        """
        Returns a new objective with variables replaced by affine substitutions.

        :param substitutions: substitutions to apply
        :type substitutions: dict[Variable, AffineSubstitution]
        :return: a new substituted objective
        :rtype: Objective
        """
        expression, _ = self.expression.substitute(substitutions)
        return Objective(expression, self.type, self.coefficient)
