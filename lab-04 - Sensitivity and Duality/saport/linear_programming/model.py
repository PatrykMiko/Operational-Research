...
class Model:
    ...
    def create_variable(
        self, name: str, lb: Optional[float] = 0, ub: Optional[float] = None
    ) -> Variable:
        """
        Returns a new variable with a specified name.
        The variable is automatically indexed and added to the model.
        Variable name has to be unique, if the name is already used, a suffix is added to make it unique.
        Bounds of the variable can be specified, but by default it is set to [0, inf).

        :param name: name of the variable
        :type name: str
        :param lb: lower bound of the variable, defaults to 0
        :type lb: Optional[float], optional
        :param ub: upper bound of the variable, defaults to None
        :type ub: Optional[float], optional
        :return: a new variable
        :rtype: Variable
        """
        new_index = len(self.variables)

        used_names = set(var.name for var in self.variables)
        while name in used_names:
            name += f"_{new_index}"

        variable = Variable(name, new_index, Domain(lb, ub))
